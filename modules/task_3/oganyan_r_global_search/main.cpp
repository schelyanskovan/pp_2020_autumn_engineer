...
void CreateTest(const std::function<double(double,double)>& func, double x_left,
    double x_right, double y_left, double y_right,
    double ans_minimum, dpair ans_pos) {
    int rank;
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);
    double time_start { MPI_Wtime()};
    const d_dpair res_paral { ParallelGlobalSearch(func, x_left, x_right, y_left, y_right) };
    if (rank == 0) {
    ...
    if (rank == 0) {
        ...
        const d_dpair res_seq { SequentialGlobalSearch(func, x_left, x_right, y_left, y_right) };
...
TEST(Parallel_Operations_MPI, Test_first_fun) {
    const std::function<double(double, double)> func { fun_first };
...
TEST(Parallel_Operations_MPI, Test_second_fun) {
    const std::function<double(double, double)> func = fun_second;
    CreateTest(func, -3.0, 3.0, -2.0, 6.0,
        -2.0, {0.0, 0.1});
}

TEST(Parallel_Operations_MPI, Test_third_fun) {
    const std::function<double(double, double)> func = fun_third;
...
TEST(Parallel_Operations_MPI, Test_forth_fun) {
    // не доделал
    const std::function<double(double, double)> func = fun_forth;
    CreateTest(func, -3.0, 3.0, -2.0, 6.0, 0.0, {0.0, 0.0});
