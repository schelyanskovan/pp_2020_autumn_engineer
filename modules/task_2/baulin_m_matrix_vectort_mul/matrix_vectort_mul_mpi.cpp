...
std::vector<int> getSequentialOperations(std::vector<int> mat, int rows,
                                         int cols, std::vector<int> vec) {
...
std::vector<int> getParallelOperations(std::vector<int> mat, int rows, int cols,
                                       std::vector<int> vec) {
  int size, rank;
  ...
  if (rank == 0) {
    for (int proc = 1; proc < size; proc++) {
      MPI_Send(&mat[proc * delta * cols], delta * cols, MPI_INT, proc, 0,
               MPI_COMM_WORLD);
    }
  }
  ...
  if (rank == 0) {
    ...
    MPI_Status status;
    MPI_Recv(&local_mat[0], delta * cols, MPI_INT, 0, 0, MPI_COMM_WORLD,
             &status);
  }
  ...
  MPI_Gather(&local_result[0], delta, MPI_INT, &global_result[0], delta,
             MPI_INT, 0, MPI_COMM_WORLD);

  if (rank == 0 && rows > size * delta) {
    std::vector<int> local_result_tail;
    local_mat = std::vector<int>(local_mat.begin() + (size * delta * cols),
                                 local_mat.end());
    local_result_tail =
        getSequentialOperations(local_mat, rows - (size * delta), cols, vec);
